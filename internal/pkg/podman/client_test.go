...
func TestClientBIListByObjectParsesFixture(t *testing.T) {
	...
	require.Equal(t, "test.txt", plain.Name())
	require.Empty(t, plain.Instance())
	require.Equal(t, 0, plain.VersionedEpoch())
	require.True(t, plain.Pending())
	...
	require.Equal(t, "test.txt", instance.Name())
	require.Equal(t, "PDGqmtJA7imna.RLH.1nsBhSy1ZWf9m", instance.Instance())

	olh, okOLH := entries[3].(*domain.OLH)
	require.True(t, okOLH)
	require.Equal(t, "\\x801001_test.txt", olh.IDX())
