...
func TestServiceObjectInspectExposesRawObjectsWithoutCopy(t *testing.T) {
	t.Parallel()
	...
	require.NoError(t, err)
	require.Len(t, result.RawObjects, 2)

	original := result.RawObjects[0]
	replacement := flow.NewRawObjectExistence(original.Name(), false)
